...
class HoverUKFConfig:
    # Conservative low-noise tuning for calm flight
    pos_process_var: float = 1e-5
    vel_process_var: float = 5e-3
    att_process_var: float = 1e-5
    vio_pos_var: float = 0.05 ** 2


class HoverUKF:
    def __init__(self, cfg: HoverUKFConfig | None = None) -> None:
        self.cfg = cfg or HoverUKFConfig()
